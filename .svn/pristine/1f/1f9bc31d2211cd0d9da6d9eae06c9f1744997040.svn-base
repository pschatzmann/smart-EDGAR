...
import ch.pschatzmann.edgar.reporting.company.FilterQuarterly;
 ...
 */
@Path("/edgar/db")
public class EdgarDBService implements Serializable {
	...
	@Produces({ MediaType.APPLICATION_JSON, MediaType.TEXT_PLAIN, MediaType.TEXT_HTML, "application/x-sql" })
	public Response valuesPost(@Context HttpHeaders headers, Query query) throws DataException, FormatException, ClassNotFoundException, SQLException {
		long start = System.currentTimeMillis();
	...
	protected Table createTable(Query query) throws DataException {
		Table table = new Table();
		table.setValueField(model.getTable("values").getValueField());
	...
	protected String getFormat(MediaType mediaType) {
	 ...
	 * @return
	 */
	protected IFieldModel getFieldModel(String col) {
	...
	protected void parameterFilter(String parameter) throws DataException {
	...
	protected void parameterColumns(String col, List<WSField> cols, Table table) throws DataException {
	...
	protected void parameterRows(String row, List<WSField> rows, Table table) throws DataException {
	...
	protected void parameterFilter(List<WSField> filter) throws DataException {
		if (!filter.isEmpty()) {
	...
	protected String getKey(String tableName, String fieldName) {
		WSField wsField1 = new WSField(tableName, fieldName);
		String key = wsField1.toString();
		return key;
	}

	protected String getKey(UriInfo uriInfo) {
	 ...
	 * @return
	 */
	protected ITableFormatter getFormatter(String format) {
	...
	public Response companyValues(@Context HttpHeaders headers, @QueryParam("format") String requestedFormat,
		...
		CompanyValues req = new CompanyValues();
		req.setFormat(requestedFormat);
		req.setPeriods(periods);
		req.setParameters(parameterNames);
		req.setCompanySelection(new CompanySelection().setCompanyNumber(identifier).setTradingSymbol(tradingSymbol));
	...
	@Path("companyValues")
	@Produces({ MediaType.APPLICATION_JSON, MediaType.TEXT_PLAIN, MediaType.TEXT_HTML, "application/x-sql" })
	public Response companyValuesPost(@Context HttpHeaders headers, CompanyValues req, @Context UriInfo uriInfo)
			throws DataException, ClassNotFoundException, SQLException, FormatException {
		...
		if (table == null) {
			...
			CompanyEdgarValuesBase companyValues = new CompanyEdgarValuesDB(req.getCompanySelection())
					.setFilter(filter, true)
					.setParameterNames(req.getParameters())
					.addFormulas(toTuples(req.getFormulas()))
					.setUnitRef(req.getUnitRef())
					.removeParameterNames(req.getRemoveParameterNames());
	...
	protected List<Tuple<String, String>> toTuples(List<String> formulas) {
		if (formulas==null) return Collections.emptyList();
		return formulas.stream().map(s -> s.split(":")).map(a -> new Tuple<String,String>(a[0].trim(),a[1].trim())).collect(Collectors.toList());
	}
